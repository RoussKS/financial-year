...
abstract class BasicEnum
{
    ...
    /**
     * Compare this enumerator against another and check if it's not the same.
     *
     * @param mixed $enumerator
     * @return bool
     */
    final public function isNot($enumerator)
    {
        return ! $this->is($enumerator);
    }

    /**
     * Check if this enumerator is one of the given enums.
     *
     * @param mixed $allowed
     * @return bool
     */
    final public function in($allowed)
    {
        if ( ! \is_array($allowed)) {
            $allowed = [$allowed];
        }
    ...
    /**
     * Check if this enumerator is not one of the given enums.
     *
     * @param mixed $except
     * @return bool
     */
    final public function notIn($except)
    {
        if ( ! \is_array($except)) {
            $except = [$except];
        }
